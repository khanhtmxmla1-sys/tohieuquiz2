...
// ONLY PUSH READING QUESTIONS THIS TIME
const TARGET_FILES = ['reading_tf.json'];

async function pushQuestions() {
    console.log('🚀 Starting import to Google Sheet (Targeted)...');
    ...
    const files = fs.readdirSync(BANK_DIR).filter(f => TARGET_FILES.includes(f));
    let totalSuccess = 0;
    ...
    console.log(`\n✨ Targeted import completed! Total added: ${totalSuccess}`);
}
