...
import { Loader2, Play, Trophy, Star, BookOpen, Clock, Target, CalendarDays, Rocket, ShieldCheck, Camera, Gift, KeyRound, Sparkles, CheckCircle2, Lock, Flame, Medal, Radio, ChevronDown } from 'lucide-react';
...
const StudentDashboardUI: React.FC<StudentDashboardUIProps> = ({ ioeQuizzes = [] }) => {
    ...
    const missionsSectionRef = useRef<HTMLElement | null>(null);
    const homeworkSectionRef = useRef<HTMLDivElement | null>(null);

    const scrollToSection = (target: React.RefObject<HTMLElement | HTMLDivElement | null>) => {
    ...
    const [isTodayJourneyExpanded, setIsTodayJourneyExpanded] = useState(false);
    const [isWeeklyQuestsExpanded, setIsWeeklyQuestsExpanded] = useState(false);
        ...
        <div className="min-h-screen bg-[#F8F9FF] text-[#0B1C30] font-sans flex">
            ...
            <aside className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 bg-[#EFF4FF] shadow-md py-6 z-50">
                <div className="px-6 mb-10">
                    <h1 className="text-2xl font-black text-[#004AC6]">TôHiệuQuiz</h1>
                </div>
                ...
                <nav className="flex-grow space-y-1 px-2">
                    ...
                    <div onClick={() => scrollToSection(missionsSectionRef)} className="text-[#434655] flex items-center p-3 rounded-xl hover:bg-[#D3E4FE] transition-all cursor-pointer">
                    ...
                    <div onClick={() => scrollToSection(homeworkSectionRef)} className="text-[#434655] flex items-center p-3 rounded-xl hover:bg-[#D3E4FE] transition-all cursor-pointer">
            ...
            <main className="md:ml-64 transition-all duration-300 min-h-screen flex-1">
                {/* --- TOP NAVBAR --- */}
                <header className="sticky top-0 z-40 bg-white shadow-sm flex items-center w-full px-4 md:px-10 py-4">
                    <div className="flex items-center gap-8">
                        ...
                        <nav className="hidden md:flex items-center gap-3">
                            ...
                            <button type="button" onClick={() => setIsJoinLiveExamModalOpen(true)} className="rounded-xl px-3 py-2 text-sm font-bold text-[#27344D] flex items-center gap-2 transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#EAF2FF] hover:text-[#004AC6] hover:shadow-sm">
                ...
                <section className="grid grid-cols-1 xl:grid-cols-[1.7fr_1fr] gap-6">
                    <div className="bg-white rounded-[28px] border border-slate-100 shadow-sm p-5 md:p-6">
                        <div 
                            onClick={() => setIsTodayJourneyExpanded(!isTodayJourneyExpanded)}
                            className="flex items-center justify-between gap-4 mb-5 cursor-pointer group"
                        >
                            <div className="flex items-center gap-3">
                                <div className="bg-violet-100 p-2 rounded-2xl text-violet-600 group-hover:scale-105 transition-transform"><Sparkles className="w-5 h-5" /></div>
                                <div>
                                    <h2 className="text-xl md:text-2xl font-black text-slate-800 group-hover:text-violet-600 transition-colors">Hành trình hôm nay</h2>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <motion.div
                                    animate={{ rotate: isTodayJourneyExpanded ? 180 : 0 }}
                        ...
                        <div 
                            ...
                            className="flex items-center justify-between gap-3 mt-6 cursor-pointer group"
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 group-hover:text-purple-600 transition-colors">Nhiệm vụ tuần</h3>
                                    <p className="text-xs text-slate-500">Reset mỗi thứ 2</p>
                                </div>
                            </div>
                            <motion.div
                                animate={{ rotate: isWeeklyQuestsExpanded ? 180 : 0 }}
                                transition={{ duration: 0.3 }}
                ...
                <section ref={missionsSectionRef} className="scroll-mt-24">
                ...
                <div ref={homeworkSectionRef} className="scroll-mt-24">
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.05)] px-6 py-3 flex justify-between items-center z-50">
                ...
                <div onClick={() => scrollToSection(missionsSectionRef)} className="flex flex-col items-center gap-1 text-[#434655] cursor-pointer">
