// ============ CONFIGURATION ============
// Token Ä‘Æ°á»£c hardcode Ä‘á»ƒ trÃ¡nh lá»—i cáº¥u hÃ¬nh
...
// - username: TÃªn Ä‘Äƒng nháº­p (VD: "gv1", "admin")
// - password: Máº­t kháº©u (plain text cho app Ä‘Æ¡n giáº£n)
// - fullName: Há» tÃªn Ä‘áº§y Ä‘á»§ (VD: "Nguyá»…n Thá»‹ Lan")
// - role: "admin" hoáº·c "teacher"
// - class: Lá»›p phá»¥ trÃ¡ch (VD: "3", "4A", "5") - GiÃ¡o viÃªn chá»‰ táº¡o Ä‘á» cho lá»›p nÃ y
//
...
function handleRequest(e) {
    ...
    // TÄƒng timeout lÃªn 60 giÃ¢y Ä‘á»ƒ xá»­ lÃ½ Ä‘á» 100 cÃ¢u há»i
    lock.tryLock(60000);

    try {
        // 1. Láº¥y tham sá»‘ (há»— trá»£ cáº£ GET vÃ  POST)
        const params = e.parameter || {};
        const postData = e.postData ? JSON.parse(e.postData.contents) : {};

        // Gá»™p tham sá»‘ Ä‘á»ƒ xá»­ lÃ½ thá»‘ng nháº¥t (dÃ¹ng Object.assign cho GAS compatibility)
        ...
        // 3. Routing (Äiá»u hÆ°á»›ng xá»­ lÃ½)
        const action = data.action;
        const sheet = SpreadsheetApp.getActiveSpreadsheet();

        switch (action) {
            case 'get_teachers':
                return getSheetData(sheet, 'Teachers'); // TÃªn tab sheet giÃ¡o viÃªn
            case 'get_quizzes':
                return getSheetData(sheet, 'Quizzes');  // TÃªn tab sheet Ä‘á» thi
            case 'get_questions':
                return getSheetData(sheet, 'Questions'); // TÃªn tab sheet cÃ¢u há»i
            case 'get_results':
                return getSheetData(sheet, 'Results');  // TÃªn tab sheet káº¿t quáº£
            case 'submit_result':
                return saveResult(sheet, data);         // Logic ghi káº¿t quáº£
            case 'create_quiz':
                return saveQuiz(sheet, data);           // Logic táº¡o Ä‘á» thi
            case 'update_quiz':
                return updateQuiz(sheet, data);
            case 'delete_quiz':
                return deleteQuiz(sheet, data.quizId);
            case 'validate_answers':
                return validateAnswers(sheet, data);  // ðŸ” Server-side answer validation
            case 'get_announcement':
                return getAnnouncement(sheet);  // ðŸ“¢ Get announcement for marquee
            case 'save_announcement':
                return saveAnnouncement(sheet, data);  // ðŸ“¢ Save announcement (admin only)
            default:
...
function validateToken(token) {
    // So sÃ¡nh token gá»­i lÃªn vá»›i token trong Script Properties
    // Náº¿u chÆ°a set trong Properties, so sÃ¡nh vá»›i fallback (khÃ´ng khuyáº¿n khÃ­ch dÃ¹ng fallback lÃ¢u dÃ i)
    return token === API_SECRET_TOKEN;
}

// HÃ m Ä‘á»c dá»¯ liá»‡u tá»« Sheet tráº£ vá» JSON (Thay tháº¿ CSV export)
function getSheetData(spreadsheet, sheetName) {
    ...
    const result = rows.map(function (row) {
        ...
        headers.forEach(function (header, index) {
            // Logic Ä‘áº·c biá»‡t: KhÃ´ng tráº£ vá» cá»™t Password náº¿u lÃ  request thÆ°á»ng
            // Hoáº·c chá»‰ tráº£ vá» password hash (náº¿u Ä‘Ã£ nÃ¢ng cáº¥p)
...
// HÃ m chá»‘ng Formula Injection (Sanitize Input)
function sanitizeInput(str) {
    if (typeof str !== 'string') return str;
    // Náº¿u báº¯t Ä‘áº§u báº±ng =, +, -, @ thÃ¬ thÃªm dáº¥u '
    if (/^[\=\+\-\@]/.test(str)) {
        return "'" + str;
    }
    return str;
...
function saveResult(sheet, data) {
    ...
    resultSheet.appendRow([
        ...
        data.totalQuestions,
        data.submittedAt,
        JSON.stringify(data.answers || {})  // ðŸ‘ˆ Cá»™t má»›i: LÆ°u answers dáº¡ng JSON
    ]);
...
function saveQuiz(sheet, data) {
    ...
    quizSheet.appendRow([
        ...
        data.classLevel,
        data.category || "",  // Danh má»¥c: vioedu, trang-nguyen, on-tap
        data.timeLimit,
        data.createdAt,
        data.accessCode || "",
        data.requireCode ? "TRUE" : "FALSE",
        sanitizeInput(data.createdBy || "") // TÃªn giÃ¡o viÃªn táº¡o Ä‘á»
    ]);
    ...
    const questionRows = questions.map(function (q) {
        ...
        return [
            ...
            blanksField,
            distractorsField,
            imageField  // ðŸ–¼ï¸ Image URL for IMAGE_QUESTION type
        ];
    });
...
// ============ ANSWER VALIDATION ============
// ðŸ” Server-side validation to prevent cheating
    ...
    quizQuestions.forEach(function (row) {
        ...
        if (qType === 'MCQ' || qType === 'SHORT_ANSWER' || qType === 'IMAGE_QUESTION') {
            ...
            try {
                ...
                // FIX: DRAG_DROP tá»« Client gá»­i lÃªn dáº¡ng Object { index: "word" }, cáº§n convert sang Array theo thá»© tá»±
            ...
            // Support CATEGORIZATION (PhÃ¢n loáº¡i)
            try {
                ...
                isCorrect = itemsData.length > 0 && itemsData.every(function (item) {
                    // Náº¿u item cÃ³ categoryId gÃ¡n sáºµn (Ä‘Ã¡p Ã¡n), check xem HS cÃ³ xáº¿p Ä‘Ãºng khÃ´ng
                    if (item.categoryId) {
